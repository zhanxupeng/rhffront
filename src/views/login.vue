        ...
        <div class="loginBox">
          ...
          <Form ref="logindata" :model="logindata" :rules="rulelogin">
            <FormItem prop="name">
              ...
              <Input v-model="logindata.name" placeholder="请输入账号" :maxlength="20" @on-keyup="replaceStr('logindata.name',$event)" @on-keydown="KeyDown" @on-blur="ifNeetCaptcha(logindata.name)" />
            </FormItem>
            <FormItem prop="pwd">
              <i class="icon icon-mima"></i>
              <Input v-model="logindata.pwd" type="password" :maxlength="20" placeholder="请输入密码" @on-keyup="replaceStr('logindata.pwd',$event)" @on-keydown="KeyDown" />
            </FormItem>
...
export default {
  ...
  methods: {
    ...
    resize() {
      ...
      document.getElementsByClassName("loginBox")[0].style.marginTop =
        (H - h) / 2 + "px";
    },
    // 验证是否需要验证码
    ifNeetCaptcha(id) {
    ...
    getVerify() {
      ...
      this.codeimg =
        "/api" + "/websys/userlogin/getCaptchaImage.pub?" + Math.random();
